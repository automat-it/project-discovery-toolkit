#!/usr/bin/env bash
# =============================================================================
# Run the full SQL Server performance audit against every user database on an
# instance. Thin wrapper around run_audit.sh that enumerates user databases
...
OUT="$OUT_ROOT/mssql_perf_all_$TS"
mkdir -p "$OUT"

# -----------------------------------------------------------------------------
# Enumerate user databases. Skip system DBs (database_id <= 4), the replication
# distribution DB, offline/restoring/standby states, and AG secondaries where
# reads are disallowed (those would fail on USE). INCLUDE pattern via LIKE.
# -----------------------------------------------------------------------------
...
echo "================================================================================"
echo "SQL Server performance audit — multi-database"
...
# Server-level pass first (wait stats / backups / AG state — DB-independent).
echo ""
...

{
    echo "--------------------------------------------------------------------------------"
    echo "Engine:      mssql"
    echo "Category:    perf (all databases)"
    echo "Timestamp:   $TS"
