...
OUT="$OUT_ROOT/mssql_perf_$TS"
mkdir -p "$OUT"

echo "================================================================================"
echo "SQL Server performance audit"
...
for priority in critical high medium low; do
    ...
    for f in "$SCRIPT_DIR/$priority"/*.sql; do
        ...
        # -C trust server cert (containers ship self-signed); -b: exit non-zero
        # on severity >= 11; sqlcmd reads the password from SQLCMDPASSWORD env.
