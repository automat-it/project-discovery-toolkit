...
# Sort + cache unique-db count once
...
if ($serverFindings.Length -eq 0) {
    ...
    foreach ($a in $serverFindings) {
        ...
        Add-To $sb "<tr class='sev-$sevC'><td><span class='badge $sevC'>$($a.Severity)</span></td><td><strong>$(Esc $a.Title)</strong><br><span class='detail'>$(Esc $a.Recommendation)</span></td><td>$($a.UniqueDbsCached.Count)</td><td class='compl'>$(Esc $a.CIS)</td><td class='compl'>$(Esc $a.GDPR)</td><td class='compl'>$(Esc $a.SOC2)</td></tr>"
    }
